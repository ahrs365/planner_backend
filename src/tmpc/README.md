...
This package implements Model Predictive Control (MPC) for motion planning in 2D dynamic environments using ROS/ROS2 C++. **This fork provides a pure C++ implementation without ROS dependencies**, allowing the planner to be used in standalone C++ applications.

A complete VSCode docker environment with this planner is available at https://github.com/tud-amr/mpc_planner_ws. This code is associated with the following publications:
...
- **Pure C++ Mode:** - **NEW!** Can be compiled and run without any ROS dependencies for standalone applications.
...
### Pure C++ Installation (No ROS)

**NEW!** This fork supports standalone C++ compilation without ROS dependencies.
...
- **ACADOS** - Nonlinear optimal control solver ([Installation Guide](https://docs.acados.org/installation/))
...
# 2. Initialize submodules (yaml-cpp)
git submodule update --init --recursive

# 3. Set ACADOS environment variable
...
The main executable will be built at `build/mpc_planner_main`.

**Output:**
- `mpc_trajectory.json` - Complete trajectory data (position, velocity, control inputs, timestamps)
- Terminal output with performance statistics

For more details, see [Pure C++ Usage](#pure-c-usage).
...
**NEW!** Run the planner without ROS:
...
# Or manually
export ACADOS_SOURCE_DIR=/path/to/acados
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$ACADOS_SOURCE_DIR/lib:$(pwd)/build/mpc_planner:$(pwd)/build/third_party/yaml-cpp
./build/mpc_planner_main test/config/settings.yaml
```

**Main Program Structure:**

The main program (`main.cpp`) demonstrates a complete control loop:

1. **Initialization**: Load configuration, create planner instance
2. **Scene Setup**: Create reference path and obstacles
3. **Control Loop**:
   - Solve MPC at configured frequency (default 20 Hz)
   - Update robot state using kinematic model
   - Save trajectory data
4. **Output**: Save complete trajectory to `mpc_trajectory.json`

**Customization:**

Edit `main.cpp` to customize:
- Reference path generation (`createReferencePath()`)
- Obstacle configuration (`createObstacles()`)
- Control loop parameters (`max_iterations_`, `control_frequency_`)
- State update model (`updateState()`)

**Configuration:**

The planner is configured via `test/config/settings.yaml`. Key parameters:
...
- Robot parameters (dimensions, velocity limits, etc.)
