...
namespace MPCPlanner
{
    ...
    void GuidanceConstraints::update(State &state, const RealTimeData &data, ModuleData &module_data)
    {
        ...
        std::cout << "[GC_DEBUG] GuidanceConstraints::update START" << std::endl;

        if (module_data.path == nullptr)
        {
            LOG_MARK("Path data not yet available");
            std::cout << "[GC_DEBUG] Path data not yet available, returning" << std::endl;
            return;
        }
        ...
        {
            std::cout << "[GC_DEBUG] T-MPC++ mode with n_paths=0, returning" << std::endl;
            return;
        }
        ...
        auto gc_t2 = std::chrono::steady_clock::now();
        std::cout << "[GC_DEBUG] setGoals took: " 
                  << std::chrono::duration<double, std::milli>(gc_t2 - gc_t1).count() << " ms" << std::endl;

        LOG_MARK("Running Guidance Search");
        std::cout << "[GC_DEBUG] Starting global_guidance_->Update()..." << std::endl;
        auto gc_t3 = std::chrono::steady_clock::now();
        bool guidance_success = global_guidance_->Update(); /** @note The main update */
        auto gc_t4 = std::chrono::steady_clock::now();
        std::cout << "[GC_DEBUG] global_guidance_->Update() took: " 
                  << std::chrono::duration<double, std::milli>(gc_t4 - gc_t3).count() << " ms"
                  << " success=" << (guidance_success ? "true" : "FALSE")
                  << " num_trajectories=" << global_guidance_->NumberOfGuidanceTrajectories()
                  << std::endl;

        mapGuidanceTrajectoriesToPlanners();

        auto gc_t5 = std::chrono::steady_clock::now();
        std::cout << "[GC_DEBUG] GuidanceConstraints::update TOTAL: " 
                  << std::chrono::duration<double, std::milli>(gc_t5 - gc_t0).count() << " ms" << std::endl;
...
#pragma omp parallel for num_threads(8)
    ...
    void GuidanceConstraints::onDataReceived(RealTimeData &data, std::string &&data_name)
    {
        ...
        if (data_name == "dynamic obstacles")
        {
            ...
            std::cout << "[GC_DEBUG] onDataReceived: total obstacles = " << data.dynamic_obstacles.size() << std::endl;
            
            // 统计真实和 dummy 障碍物
            int real_count = 0, dummy_count = 0;
            for (const auto &obs : data.dynamic_obstacles) {
                if (obs.index < 0) dummy_count++;
                else real_count++;
            }
            std::cout << "[GC_DEBUG] Real obstacles: " << real_count << ", Dummy obstacles: " << dummy_count << std::endl;
            ...
            std::cout << "[GC_DEBUG] Loading " << obstacles.size() << " obstacles to global_guidance_" << std::endl;
