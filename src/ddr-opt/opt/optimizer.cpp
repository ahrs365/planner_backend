...
bool MSPlanner::minco_plan(const FlatTrajData &flat_traj){
    ...
    opt_timed_out_ = false;
    opt_time_limit_sec_ = 1.0;
    ...
    for(; replan_num_for_coll < safeReplanMaxTime; replan_num_for_coll++){
        ...
        if(optimizer()){
            ...
            return false;
        if (opt_timed_out_) {
            std::cerr << "[Optimizer] Timeout: optimization exceeded "
                      << opt_time_limit_sec_ << " s" << std::endl;
            return false;
        }
...
bool MSPlanner::optimizer(){
    ...
    if (opt_timed_out_) {
        std::cerr << "[Optimizer] Timeout: preprocessing exceeded "
                  << opt_time_limit_sec_ << " s" << std::endl;
        return false;
    }
